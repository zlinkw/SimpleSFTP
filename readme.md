...
`SimpleSFTP` 是一个本地 VS Code 扩展，用于在 Windows 本地编辑项目，并通过系统 `ssh`、`tar` 与远端 Linux 项目目录同步。当前版本为 `0.1.2`。
...
```powershell
npx --yes @vscode/vsce package --no-dependencies --out simple-sftp-0.1.2.vsix --allow-missing-repository
```

安装 VSIX：

```powershell
code --install-extension .\simple-sftp-0.1.2.vsix --force
```
